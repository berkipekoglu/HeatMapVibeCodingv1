...
export async function GET() {
  try {
    ...
    // Check for specific error codes if tables already exist (e.g., '42P07' for duplicate_table)
    if (error.code === '42P07') { // duplicate_table
        return NextResponse.json({ success: false, message: 'Database already initialized (tables exist).' }, { status: 409 });
    }
    return NextResponse.json({ success: false, message: 'Database initialization failed.', error: error.message }, { status: 500 });
  }
}